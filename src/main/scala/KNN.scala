...
object KNN {
  def main(args: Array[String]): Unit = {
    ...
    // accuracy
    LOGGER.info(accuracy(result, ytest.collect()))

    // precision
    LOGGER.info(precision(result, ytest.collect()))

    // recall
    LOGGER.info(recall(result, ytest.collect()))

    //f1
    LOGGER.info(f1_score(result, ytest.collect()))
    ...
    for (loss <- losses) {
      ...
      for (neighbors <- 1 to 10) {
        ...
        LOGGER.info(cross_validate(model, data, categories, 5, loss))
      }
    }
